...
title(['Piecewise Model $I_{D}$-$V_{D}$ characteristics for $t_{ox}$ = ', num2str(tox*1e9), ' nm, L = ', num2str(L*1e6),' $\mu$m'], 'interpreter', 'latex');
...
for Vds = Vds_arr
    ...
    for Vgs = Vgs_arr
        Id = 0;
        if(Vgs < Vth)
            Id = u_n_fixed * (W/L) * Cox * (m - 1) *VT^2 * exp(q*(Vgs - Vth)/(m*kB*T))*(1 - exp(-q*Vds/(kB*T)));
        else
            Vd_sat = (Vgs - Vth)/m;

            if(Vds < Vd_sat)
                Id = u_n_fixed * Cox * (W/L) * (Vgs - Vth - 0.5*m*Vds) * Vds;
            else
                Id = u_n_fixed * Cox * (W/L) * (Vgs - Vth)^2/(2*m);
            end
        end
        Id_vals = [Id_vals Id];
    end
...
ylabel('$I_{DS}$ (in A)', 'interpreter', 'latex');
title(['Piecewise Model $I_{D}$-$V_{G}$ characteristics for $t_{ox}$ = ', num2str(tox*1e9), ' nm, L = ', num2str(L*1e6),' $\mu$m'], 'interpreter', 'latex');
